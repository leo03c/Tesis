...
const GameCarouselSection: React.FC<GameCarouselSectionProps> = ({
  title,
  games,
  loading = false,
  error = null,
  ...
  if (loading) {
    return (
      <div className="my-4 rounded-3xl bg-deep text-white py-10 px-6">
        <h2 className="text-xl font-primary mb-6">{title}</h2>
          ...
          <div className="hidden sm:flex items-center gap-2">
            ...
            <button
              ...
              className={`p-2 rounded-full ${
                currentPage === 0 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-800"
              }`}
            >
              ...
              <Link key={juego.id} href={`/juego/${juego.slug}`}>
                <div className="bg-deep rounded-xl overflow-hidden md:shadow-md relative cursor-pointer hover:scale-105 transition-transform duration-300">
                  <div className="w-full aspect-[4/3] relative">
                    ...
                    />
                    {showFavorite && isFavorite && onToggleFavorite && (
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          onToggleFavorite(juego.id);
                        }}
